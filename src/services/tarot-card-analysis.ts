 ...
 */
export async function analyzeTarotCards(question: string, cardsToAnalyze: { name: string; isReversed: boolean }[]): Promise<TarotCard> {
  console.log("Sending to API:", JSON.stringify({ question, TarotCardData: cardsToAnalyze })); // Log request body
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Error fetching tarot card interpretations:', response.status, response.statusText, errorBody);
      throw new Error(`API Error: ${response.status} ${response.statusText} - ${errorBody}`); // Throw an error to be caught below
    }
